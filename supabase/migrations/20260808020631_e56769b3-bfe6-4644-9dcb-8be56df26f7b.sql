ALTER TABLE public.reservas DROP CONSTRAINT IF EXISTS reservas_tipo_chk;
ALTER TABLE public.reservas ADD CONSTRAINT reservas_tipo_chk CHECK (tipo = ANY (ARRAY['locacao'::text,'pacote_marcas'::text,'producao'::text]));
...
BEGIN
  ...
  INSERT INTO public.cobrancas (
    ...
    v_sinal, 'contato.mambaia@gmail.com', 'LAURA OTTONI NEGRÃO',
...
REVOKE ALL ON FUNCTION public.criar_reserva_producao(date, time without time zone, text, text, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.criar_reserva_producao(date, time without time zone, text, text, text, text) TO anon, authenticated, service_role;
...
BEGIN
  ...
  v_cat := CASE WHEN NEW.tipo IN ('pacote_marcas','producao')
                THEN '0f20966a-9361-4579-9cb4-33f0c9db13e7'::uuid
                ELSE '197f2885-627f-4fcb-b4a5-daaeb19b09d9'::uuid
           END;
  v_valor := COALESCE(NEW.valor_pago, NEW.valor_sinal);
  v_desc  := CASE WHEN NEW.tipo = 'pacote_marcas' THEN 'Pacote Marcas'
                  WHEN NEW.tipo = 'producao' THEN COALESCE(NEW.observacoes, 'Pacote de produção')
                  ELSE 'Locação estúdio' END
             || ' — ' || NEW.cliente_nome
