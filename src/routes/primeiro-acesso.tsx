...
function PrimeiroAcesso() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (profile) {
      await supabase.from("profiles").update({ display_name: name.trim() || profile.display_name, must_change_password: false }).eq("id", profile.id);
    }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 bg-background">
      <Card className="w-full max-w-sm p-6">
        ...
        <p className="text-sm text-muted-foreground mb-5">Defina sua senha e como quer ser chamado.</p>
        <form onSubmit={onSubmit} className="space-y-3">
          ...
          <div>
            ...
            <Input id="p1" type="password" value={pwd} onChange={(e) => setPwd(e.target.value)} required minLength={6} autoComplete="new-password" />
          </div>
          <div>
            <Label htmlFor="p2">Confirme a senha</Label>
            <Input id="p2" type="password" value={pwd2} onChange={(e) => setPwd2(e.target.value)} required minLength={6} autoComplete="new-password" />
          </div>
