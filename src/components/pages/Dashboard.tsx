...
export function Dashboard() {
  const { data: transacoes, isLoading: l1 } = useQuery({
    queryKey: ["transacoes"],
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Considerando despesas ainda não acertadas
            </p>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {socios.map((s) => {
                ...
                return (
                  <div
                    key={s.id}
                    className="rounded-lg border border-border p-3 flex items-center justify-between bg-background"
                  >
                    ...
                    <div
                      className={`text-sm font-semibold tabular-nums ${v > 0 ? "text-[color:var(--success)]" : v < 0 ? "text-destructive" : "text-muted-foreground"}`}
                    >
                      {v > 0 ? "+" : ""}
                      {formatBRL(v)}
                    </div>
                    ...
                    return (
                      <div key={i} className="text-sm flex items-center gap-2">
                        ...
                        <span className="ml-auto font-semibold tabular-nums">
                          {formatBRL(fromCents(s.valor_cents))}
                        </span>
                      </div>
                    );
                  })}
        ...
        <div className="space-y-1">
          {transacoes!.slice(0, 8).map((t) => {
            ...
            return (
              ...
              >
                <div
                  className={`w-9 h-9 rounded-lg flex items-center justify-center text-xs ${t.tipo === "receita" ? "bg-[color:var(--brand-lime)] text-[color:var(--brand-dark)]" : "bg-[color:var(--brand-dark)] text-[color:var(--brand-lime)]"}`}
                >
                  {t.tipo === "receita" ? (
                ...
                <div
                  className={`text-sm font-semibold tabular-nums ${t.tipo === "receita" ? "text-[color:var(--success)]" : "text-foreground"}`}
                >
                  {t.tipo === "receita" ? "+" : "−"}
                  {formatBRL(t.valor)}
                </div>
          ...
          {transacoes!.length === 0 && (
            <div className="text-center py-8 text-sm text-muted-foreground">
              ...
              <Link to="/nova" className="text-primary underline">
  ...
  return (
    <Card className="p-4 md:p-5">
      <div className="flex items-center justify-between mb-2">
        ...
        <span
          className={`w-7 h-7 rounded-md flex items-center justify-center ${toneClasses[tone]}`}
        >
          {icon}
        </span>
      </div>
