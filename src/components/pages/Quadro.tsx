...
export function Quadro() {
  ...
  const { data: itens, isLoading } = useQuery({
    queryKey: ["quadro_itens"],
    queryFn: fetchQuadroItens,
  });
  ...
  const filtrados = useMemo(() => {
    ...
    return itens.filter((i) => {
      ...
      if (
        busca &&
        !`${i.titulo} ${i.descricao ?? ""} ${i.tags.join(" ")}`
          .toLowerCase()
  ...
  const moverStatus = useMutation({
    mutationFn: ({ id, status }: { id: string; status: QuadroStatus }) =>
      updateQuadroItem(id, { status }),
  ...
  const quickUpdate = useMutation({
    mutationFn: ({ id, patch }: { id: string; patch: Partial<QuadroItem> }) =>
      updateQuadroItem(id, patch),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["quadro_itens"] }),
    onError: (e) => toast.error(friendlyErrorMessage(e)),
  });
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {porColuna("arquivado").map((item) => (
            <ItemCard
              ...
              onQuick={(patch) => quickUpdate.mutate({ id: item.id, patch })}
            />
          ))}
          ...
          <div className="grid md:grid-cols-3 gap-4">
            {COLUNAS.map((col) => (
              <Coluna
                ...
                status={col.status}
                label={col.label}
                count={porColuna(col.status).length}
              >
...
function ItemCard({
  ...
  return (
    <Card
      ...
      style={{
        borderLeftColor:
          `var(--brand-${item.tipo === "ideia" ? "lime" : item.tipo === "tarefa" ? "green" : "dark"})` as string,
      }}
    >
      <div
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <Select
                value={form.tipo}
                onValueChange={(v) => setForm({ ...form, tipo: v as QuadroTipo })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(v) => setForm({ ...form, prioridade: v as QuadroPrioridade })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <Select
                value={form.status}
                onValueChange={(v) => setForm({ ...form, status: v as QuadroStatus })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                ...
                <button
                  ...
                  onClick={() =>
                    setForm({
                      ...form,
                      responsavel_ids:
                        form.responsavel_ids.length === socios.length
                          ? []
                          : socios.map((s) => s.id),
                    })
                  }
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <Select
                ...
                onValueChange={(v) => setForm({ ...form, empresa: v === "none" ? "" : v })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
          ...
          <div>
            ...
            <Input
              ...
              placeholder="urgente, evento, foto"
            />
          </div>
        </div>
            ...
            {state.mode === "edit" && form.id && (
              <>
                <Button
                  ...
                  size="sm"
                  onClick={() => {
                    setForm({ ...form, status: "arquivado" });
                    onSave({ ...form, status: "arquivado" });
                  }}
                >
