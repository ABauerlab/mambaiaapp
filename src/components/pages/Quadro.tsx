...
const EMPRESAS = ["Mambaia", "Kriya", "Kodara", "Estúdio"];
      ...
      {isLoading ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {porColuna("arquivado").map((item) => (
            <ItemCard key={item.id} item={item} socios={socios ?? []} onClick={() => abrirEdicao(item)} />
          ))}
            ...
            {COLUNAS.map((col) => (
              <Coluna key={col.status} status={col.status} label={col.label} count={porColuna(col.status).length}>
                {porColuna(col.status).map((item) => (
                  <DraggableCard key={item.id} item={item} socios={socios ?? []} onClick={() => abrirEdicao(item)} />
                ))}
              </Coluna>
            ))}
          </div>
...
function DraggableCard({ item, socios, onClick }: { item: QuadroItem; socios: { id: string; nome: string; cor: string }[]; onClick: () => void }) {
  const { attributes, listeners, setNodeRef, transform, isDragging } = useDraggable({ id: item.id });
  const style = transform ? { transform: `translate3d(${transform.x}px, ${transform.y}px, 0)` } : undefined;
  return (
    <div ref={setNodeRef} style={style} {...attributes} {...listeners} className={isDragging ? "opacity-50" : ""}>
      <ItemCard item={item} socios={socios} onClick={onClick} />
    </div>
  );
}

function ItemCard({ item, socios, onClick }: { item: QuadroItem; socios: { id: string; nome: string; cor: string }[]; onClick: () => void }) {
  const tipo = TIPOS.find((t) => t.value === item.tipo)!;
  const Icon = tipo.icon;
  const responsavel = socios.find((s) => s.id === item.responsavel_id);
  return (
    <Card
      onClick={onClick}
    ...
    >
      <div className="flex items-start justify-between gap-2 mb-1.5">
        ...
        <Badge className={PRIO_COLOR[item.prioridade]} variant="outline">
          {item.prioridade}
        </Badge>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-1.5 text-[11px] text-muted-foreground">
        {responsavel && (
          <span className="inline-flex items-center gap-1">
            <span className="w-2 h-2 rounded-full" style={{ background: responsavel.cor }} />
