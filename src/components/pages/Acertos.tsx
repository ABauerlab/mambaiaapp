  ...
  const zerarTudo = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase
        .from("transacoes")
        .update({ acertada: true })
        .in("id", result.abertasIds);
      if (error) throw error;
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["transacoes"] });
      toast.success("Movido ao balanco geral");
    },
          ...
          <Card className="p-5 mb-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <SaldoCard
                nome="Mambaia (caixa)"
                cor="#1F3D2B"
                valor={fromCents(result.bal.get(MAMBAIA_CAIXA_ID) ?? 0)}
              ...
              {socios.map((s) => (
                <SaldoCard
                  ...
                  nome={s.nome}
                  cor={s.cor}
                  valor={fromCents(result.bal.get(s.id) ?? 0)}
                />
              ))}
            </div>
          </Card>
            ...
            <div className="flex items-center justify-between mb-4 gap-2 flex-wrap">
              ...
              {result.sugestoes.length === 0 && result.abertasIds.length > 0 && (
                <Button onClick={() => zerarTudo.mutate()} disabled={zerarTudo.isPending}>
                  {zerarTudo.isPending && <Loader2 className="w-4 h-4 mr-2 animate-spin" />}Mover ao
                  balanco
                </Button>
              )}
            </div>
              ...
              <div className="space-y-2">
                {result.sugestoes.map((s) => {
                  ...
                  return (
                    <div
                      key={key}
                      className="flex items-center gap-3 p-3 rounded-lg border border-border flex-wrap"
                    >
                      ...
                      <Button
                        ...
                        onClick={() => {
                          setRegistering(key);
                          registrarAcerto.mutate({
                            de: s.de,
                            para: s.para,
                            valor_cents: s.valor_cents,
                          });
                        }}
                      >
                        {registering === key ? (
...
function SaldoCard({
  ...
  return (
    <div className="rounded-lg border border-border p-4 text-center">
      <div
        className="w-12 h-12 rounded-full mx-auto flex items-center justify-center text-sm font-bold mb-2"
        style={{ background: cor, color: "#0A2A20" }}
      >
        {icon ??
          nome
            .split(" ")
